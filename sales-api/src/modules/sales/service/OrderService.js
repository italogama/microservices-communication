...
class OrderService {
    async createOrder(req) {
        try {
            ...
            console.info(`Request to POST login with data ${JSON.stringify(
                orderData
            ...
            console.info(`Response to POST login with data ${JSON.stringify(
                response
                )} | [transactionID: ${transactionid} | serviceID: ${serviceid}]`
            );
            return response;
    ...
    async findById(req) {
        try {
            const { id }  = req.params;
            const { transactionid, serviceid } = req.headers;
            console.info(`Request to GET order by ${id} | [transactionID: ${transactionid} | serviceID: ${serviceid}]`
            );
            ...
            console.info(`Response to GET order by ID ${id}: ${JSON.stringify(
                response
                )} | [transactionID: ${transactionid} | serviceID: ${serviceid}]`
            );
            return response;
    ...
    async findAll(req) {
        try {
            ...
            console.info(`Response to GET all orders: ${JSON.stringify(
                response
                )} | [transactionID: ${transactionid} | serviceID: ${serviceid}]`
            );
            return response;
    ...
    async findByProductId(req) {
        try {
            ...
            console.info(`Response to GET orders by productID ${productId}: ${JSON.stringify(
                response
                )} | [transactionID: ${transactionid} | serviceID: ${serviceid}]`
            );
            return response;
    ...
    validateInformedProductId(id) {
        if (!id) {
            throw new OrderException(BAD_REQUEST, "The order's productId ID must be informed.");
        }
    }
}
